...
const Spreadsheet = ({
  ...
  const handleCellChange = (rowId, colId, value) => {
    ...
    if (mode === "edit") {
      ...
      ) || rowId.startsWith("new-"); // or however you distinguish local vs server rows
  ...
  const addColumn = () => {
    ...
    setTableState((prev) => ({
      ...prev,
      columns: [...prev.columns, { id, name, width: DEFAULT_COL_WIDTH }],
    }));

    if (mode === "edit") {
      enqueue({ type: "column_add", id, name }); // now name is defined
    }
  };
  ...
  const deleteRow = (rowId, rowNum) => {
    if (mode === "edit") {
      ...
      if (row?.dbRowNum) {
        const idColId = columns.find((c) => c.name === "id")?.id;
        const idValue = row.cells[idColId];
        enqueue({ type: "row_delete", rowId, idValue }); // ← store idValue instead of dbRowNum
      } else {
  ...
  const handleConfirm = async ({ datasetName }) => {
    ...
    if (mode === "create") {
      ...
      console.log("editTable result:", result); // add this

      if (!result || result.success === false) { // guard against undefined
