...
const Data = () => {
  ...
  const [datasets, setDatasets] = useState([
    { id: 1, name: "Weather Station A", type: "Weather", rows: 1520, lastModified: "2026-03-08" },
    { id: 2, name: "Battery Pack 04 Test", type: "Battery", rows: 845, lastModified: "2026-03-09" },
    { id: 4, name: "Solar Grid Analysis", type: "Energy", rows: 2100, lastModified: "2026-03-01" },
  ]);
  ...
  return (
    <div className="p-6 flex flex-col gap-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">My Datasets</h1>
        <p className="text-slate-500 text-sm">Review and manage your submitted data tables.</p>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 dark:bg-pitch border-b border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300">
              <th className="p-4 font-semibold text-xs uppercase tracking-wider">Dataset Name</th>
              <th className="p-4 font-semibold text-xs uppercase tracking-wider">Type</th>
              <th className="p-4 font-semibold text-xs uppercase tracking-wider text-center">Rows</th>
              <th className="p-4 font-semibold text-xs uppercase tracking-wider">Modified Date</th>
              <th className="p-4 font-semibold text-xs uppercase tracking-wider text-right">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {datasets.map((ds) => (
              <tr key={ds.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors group">
                {/* 1. Dataset Name */}
                ...
                <td className="p-4">
                  <div className="flex justify-end items-center gap-4">
                    <button 
                      ...
                      className="px-6 py-1.5 bg-sky text-white text-sm font-bold rounded-lg hover:bg-sky/90 transition-all shadow-sm shadow-sky/10"
                    >
                      View
                    </button>
